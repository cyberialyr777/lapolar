...
$data = obtenerProveedores($conexion);
$proveedores = $data['proveedores'];

if (isset($_GET['txtID'])) {
  $txtID = (isset($_GET['txtID'])) ? $_GET['txtID'] : "";

  $sentencia = $conexion->prepare("SELECT * FROM deudas WHERE numero_de_factura=:numero_de_factura");
  $sentencia->bindParam(":numero_de_factura", $txtID);
  $sentencia->execute();
  $registro = $sentencia->fetch(PDO::FETCH_LAZY);

  $proveedor = $registro['ID_PROVEEDOR'];
...
if ($_POST) {
  print_r($_POST);
  $proveedores = isset($_POST['id_proveedor']) ? htmlspecialchars(trim($_POST['id_proveedor'])) : "";
  $monto = isset($_POST['monto']) ? (int)$_POST['monto'] : 0;
  $fecha_limite = isset($_POST['fecha_limite']) ? htmlspecialchars(trim($_POST['fecha_limite'])) : "";
  ...
  $sentencia = $conexion->prepare("UPDATE deudas SET 
  ID_PROVEEDOR=:id_proveedor,
  MONTO=:monto,
  FECHA_LIMITE=:fecha_limite,
  FECHA_ACTUALIZACION=NOW() 
  WHERE NUMERO_DE_FACTURA=:numero_de_factura");

  $sentencia->bindParam(":numero_de_factura", $txtID);
  $sentencia->bindParam(":id_proveedor", $proveedores);
          ...
          <form action="" enctype="multipart/form-data" method="post" class="form">
            <div class="form">
              <div class="mb-3">
                ...
                <input readonly value="<?php echo $txtID; ?>" type="text" class="form-control numero_de_factura" name="numero_de_factura" id="formGroupExampleInput2" placeholder="56">
              </div>
              <select style="margin-bottom: 2%;" class="form-select" name="id_proveedor" aria-label="Seleccionar proveedor">
                <option value="" disabled>Seleccionar proveedor</option>
                <?php foreach ($proveedores as $proveedorItem) { ?>
                  <option value="<?php echo $proveedorItem['ID_PROVEEDOR']; ?>" <?php if ($proveedor == $proveedorItem['ID_PROVEEDOR']) echo "selected"; ?>><?php echo $proveedorItem['NOMBRE']; ?></option>
                <?php } ?>
              </select>
              ...
              <div class="mb-3">
                ...
                <input name="monto" value="<?php echo $monto; ?>" type="text" class="form-control" id="formGroupExampleInput2" placeholder="2344">
              </div>
              <div class="mb-3">
                <label for="formGroupExampleInput2" class="form-label">Fecha Límite</label>
                <div class="input-group date fj-date">
                  <input value="<?php echo $fecha_limite; ?>" name="fecha_limite" type="text" class="form-control"><span class="input-group-addon"><i class="glyphicon glyphicon-th"></i></span>
                </div>
              </div>
