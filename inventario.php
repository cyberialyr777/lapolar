...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $sentencia = $conexion->prepare("INSERT INTO `inventarios` (`ID_REFACCION`, `ID_MARCA`, `ID_MODELO`, `REFACCION`, `CANTIDAD`, `FECHA_ENTRADA`, `FECHA_SALIDA`) VALUES (NULL, :id_marca, :id_modelo, :refaccion, :cantidad, NOW(), NOW())");
...
$sentencia = $conexion->prepare("SELECT * FROM `inventarios`");
$sentencia->execute();
$lista_clientes = $sentencia->fetchAll(PDO::FETCH_ASSOC);

if (isset($_GET['eliminar'])) {
  $txtID = $_GET['eliminar'];
  $sentencia = $conexion->prepare("DELETE FROM inventarios WHERE ID_REFACCION = :id");
            ...
            <form action="" enctype="multipart/form-data" method="post">
              <div class="form">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control nombre" name="refaccion" id="nombre" placeholder="Banda">
                </div>
                <div class="mb-3">
                  <label for="telefono" class="form-label">Cantidad</label>
                  <input type="text" class="form-control telefono" name="cantidad" id="telefono" placeholder="9">
                </div>
        ...
        <div class="fondo container-fluid row">
          ...
          <div class="buscador col-sm-4">
            <form method="get" class="d-flex">
              <input id="search-inventario" class="form-control me-2" type="text" placeholder="Encontrar refacciones" onkeyup="buscar_inventario()" />
          ...
          <table class="table table-borderless table-striped" id="tabla_inventario">
            ...
            <tbody>
              <?php foreach ($lista_clientes as $registros) { ?>
                <tr>
                  <td><?php echo $registros['ID_REFACCION']; ?></td>
                  <td><?php echo $registros['REFACCION']; ?></td>
                  <td><?php echo $registros['CANTIDAD']; ?></td>
                  <td><?php
                      $idMarca = $registros['ID_MARCA'];
                      foreach ($marcas as $marca) {
                        if ($marca['ID_MARCA'] === $idMarca) {
                          echo $marca['MARCA']; // Mostrar el nombre del proveedor
                          break; // Salir del bucle una vez encontrado
                        }
                      }
                      ?></td>
                  <td><?php
                      $idModelo = $registros['ID_MODELO'];
                  ...
                  <td>
                    <a style="background-color: #1A132F !important;" name="" id="" class="editar btn btn-light" href="editar_inventario.php?txtID=<?php echo $registros['ID_REFACCION']; ?>" role="button">
                    ...
                    <a style="background-color: #1A132F !important;" name="" id="" class="eliminar btn btn-light" href="<?php echo $_SERVER['PHP_SELF'] . '?eliminar=' . $registros['ID_REFACCION']; ?>" role="button">
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
