...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Otros campos que necesites validar y limpiar

$archivoNombre = basename($_FILES["archivo"]["name"]);
$archivoTemp = $_FILES["archivo"]["tmp_name"];
$archivoRuta = "C:/xampp/htdocs/PROYECTO/archivos_facturas/" . $archivoNombre;


  // Validación y procesamiento del archivo
  if (move_uploaded_file($archivoTemp, $archivoRuta)) {
    // Configurar la conexión a la base de datos


    $sentencia = $conexion->prepare("INSERT INTO `facturas_clientes` (`NUMERO_DE_FACTURA`, `ID_CLIENTE`, `ARCHIVO`, `FECHA_CREACION`, `FECHA_ACTUALIZACION`) VALUES (:numero_de_factura, :id_cliente, :archivoNombre, NOW(), NOW())");
    ...
    $sentencia->bindParam(":archivoNombre", $archivoNombre);
...
$sentencia = $conexion->prepare("SELECT * FROM `facturas_clientes`");
$sentencia->execute();
$lista_facturas = $sentencia->fetchAll(PDO::FETCH_ASSOC);

if (isset($_GET['eliminar'])) {
  $txtID = $_GET['eliminar'];
  $sentencia = $conexion->prepare("DELETE FROM facturas_clientes WHERE NUMERO_DE_FACTURA = :numero_de_factura");
      ...
      <div class="col-sm-9" style="margin-bottom: 230px;">
        ...
        <div class="fondo container-fluid row">
          ...
          <div class="buscador col-sm-4">
            <form class="d-flex">
              <input class="form-control me-2" type="text" placeholder="Encontrar facturas" />
          ...
          <table class="table table-borderless table-striped">
            ...
            <tbody>
              <?php foreach ($lista_facturas as $registros) { ?>
                <tr>
                  <td><?php echo $registros['NUMERO_DE_FACTURA']; ?></td>
                  <td><?php $idCliente = $registros['ID_CLIENTE'];
                  ...
                  <td><a class="link_factura" style="color: #1A132F;" href="<?php echo $url_base . 'archivos_facturas/' . $registros['ARCHIVO']; ?>" download><?php echo $registros['ARCHIVO']; ?></a></td>
                  <td><?php echo $registros['FECHA_CREACION']; ?></td>
                  <td><?php echo $registros['FECHA_ACTUALIZACION']; ?></td>
                  <td>
                    <a style="background-color: #1A132F !important;" name="" id="" class="editar btn btn-light" href="editar_facturas.php?txtID=<?php echo $registros['NUMERO_DE_FACTURA']; ?>" role="button">
                    ...
                    <a style="background-color: #1A132F !important;" name="" id="" class="eliminar btn btn-light" href="<?php echo $_SERVER['PHP_SELF'] . '?eliminar=' . $registros['NUMERO_DE_FACTURA']; ?>" role="button">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
