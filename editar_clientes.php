...
if (isset($_GET['txtID'])) {
  ...
  $sentencia = $conexion->prepare("SELECT * FROM clientes WHERE id_cliente=:id");
  $sentencia->bindParam(":id", $txtID);
  $sentencia->execute();
  $registro = $sentencia->fetch(PDO::FETCH_LAZY);

  $nombre = $registro['NOMBRE'];
  $municipio = $registro['ID_MUNICIPIO'];
  ...
  $representante_nombre = $registro['REPRESENTANTE_NOMBRE'];
  $representante_apellido_paterno = $registro['REPRESENTANTE_APELLIDO_PATERNO'];
}

if ($_POST) {
  print_r($_POST);
  $txtID = isset($_POST['txtID']) ? htmlspecialchars(trim($_POST['txtID'])) : "";
...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="css/editar_clientes.css" />
      ...
      <div class="col-sm-9" style="margin-bottom: 230px;">
        <div class="container formulario">
          <form method="post" class="form" action="" enctype="multipart/form-data">
            <div class="mb-3">
              <label for="txtID" class="form-label id">ID</label>
              <input readonly value="<?php echo $txtID; ?>" type="text" class="form-control id" name="txtID" id="txtID" placeholder="ID">
            </div>
            <div class="mb-3">
              <label for="nombre" class="form-label">Nombre</label>
              <input value="<?php echo $nombre; ?>" type="text" class="form-control nombre" name="nombre" id="nombre" placeholder="Samurai Refacciones">
            </div>
            <div class="mb-3">
              <label for="telefono" class="form-label">Teléfono</label>
              <input value="<?php echo $telefono; ?>" type="text" class="form-control telefono" name="telefono" id="telefono" placeholder="9934563478">
            </div>
            <div class="mb-3">
              <label for="calle" class="form-label">Calle</label>
              <input value="<?php echo $calle; ?>" type="text" class="form-control calle" name="calle" id="calle" placeholder="Calle Pitahaya">
            </div>
            <div class="mb-3">
              <label for="numero" class="form-label">Numero</label>
              <input value="<?php echo $numero; ?>" type="text" class="form-control numero" name="numero" id="numero" placeholder="#41">
            </div>
            <div class="mb-3">
              <label for="cp" class="form-label">C.P.</label>
              <input value="<?php echo $cp; ?>" type="text" class="form-control cp" name="cp" id="cp" placeholder="86453">
            </div>


            <select class="form-select muni" name="id_municipio" aria-label="Seleccionar municipio">
              <option value="" disabled>Seleccionar municipio</option>
              <?php foreach ($municipios as $clienteItem) { ?>
                <option value="<?php echo $clienteItem['ID_MUNICIPIO']; ?>" <?php if ($municipios == $clienteItem['ID_MUNICIPIO']) echo "selected"; ?>><?php echo $clienteItem['NOMBRE']; ?></option>
              <?php } ?>
            </select>

            <select class="form-select esta" name="id_estado" aria-label="Seleccionar estado">
            <option value="" disabled>Seleccionar estado</option>
              <?php foreach ($estados as $clienteItem) { ?>
                <option value="<?php echo $clienteItem['ID_ESTADO']; ?>" <?php if ($estados == $clienteItem['ID_ESTADO']) echo "selected"; ?>><?php echo $clienteItem['NOMBRE']; ?></option>
              <?php } ?>
            </select>

            <select class="form-select colo" name="id_colonia" aria-label="Seleccionar colonia">
            <option value="" disabled>Seleccionar colonia</option>
              <?php foreach ($colonias as $clienteItem) { ?>
                <option value="<?php echo $clienteItem['ID_COLONIA']; ?>" <?php if ($colonias == $clienteItem['ID_COLONIA']) echo "selected"; ?>><?php echo $clienteItem['NOMBRE']; ?></option>
              <?php } ?>
            </select>
            ...
            <div class="mb-3">
              <label for="correo" class="form-label correo">Correo</label>
              <input value="<?php echo $correo; ?>" type="text" class="form-control correo" name="correo" id="correo" placeholder="samurai@gmail.com">
            </div>
            <div class="mb-3">
              <label for="representante_nombre" class="form-label">Representante nombre</label>
              <input value="<?php echo $representante_nombre; ?>" type="text" class="form-control representante_nombre" name="representante_nombre" id="representante_nombre" placeholder="Juan">
            </div>
            <div class="mb-3">
              <label for="representante_apellido_paterno" class="form-label">Representante apellido paterno</label>
              <input value="<?php echo $representante_apellido_paterno; ?>" type="text" class="form-control representante_apellido_paterno" name="representante_apellido_paterno" id="representante_apellido_paterno" placeholder="González">
            </div>
            <div>
              <button name="guardar" type="submit" class="guardar-btn btn">Guardar</button>
            </div>
          </form>
        </div>
